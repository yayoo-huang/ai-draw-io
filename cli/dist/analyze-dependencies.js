...
function inferServiceName(files) {
    if (files.length === 0)
...
function analyzeServiceDependencies(files) {
    const serviceName = inferServiceName(files);
    const dependencies = [];
    for (const file of files) {
        if (file.language === "java") {
            // @Provides pattern
            const providesPattern = /@Provides[\s\S]{0,200}?public\s+(\w+(?:Client|Service|Lambda|Authority))\s+\w+/g;
            let match;
            ...
            const clientBuilderPattern = /new\s+ClientBuilder\(\)[\s\S]{0,100}?\.remoteOf\((\w+)\.class\)/g;
            ...
            // Import statements
            const importPattern = /import\s+[\w.]+\.(\w+(?:Client|Service|Lambda|Authority))\s*;/g;
    ...
    return {
        serviceName,
        dependencies: Array.from(serviceMap.values()).sort((a, b) => a.serviceName.localeCompare(b.serviceName)),
    };
}
function generateDependencyContext(analysis) {
    const { serviceName, dependencies } = analysis;
    if (dependencies.length === 0) {
        return `# Service Dependency Analysis\n\n**Service**: ${serviceName}\n\n**Dependencies**: None detected`;
    }
    const depList = dependencies
        .map((dep, index) => `${index + 1}. **${dep.serviceName}** (${dep.confidence} confidence)`)
        .join("\n");
    return `# Service Dependency Analysis
...
function scanDirectory(dir, baseDir = dir) {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            const isCodeFile = CODE_EXTENSIONS.some((ext) => entry.name.endsWith(ext));
            if (!isCodeFile)
                continue;
            try {
 ...
 */
async function analyzeCurrentDirectory(options) {
    ...
    if (analysis.dependencies.length > 0) {
        ...
        analysis.dependencies.forEach((dep, index) => {
            console.log(`   ${index + 1}. ${dep.serviceName} (${dep.confidence})`);
            if (options.verbose) {
    ...
    // Save to file
    if (options.output) {
        (0, fs_1.writeFileSync)(options.output, JSON.stringify(analysis, null, 2));
...
`);
    ...
    const serviceName = args.find((arg) => !arg.startsWith("-") && arg !== "analyze");
    if (serviceName) {
